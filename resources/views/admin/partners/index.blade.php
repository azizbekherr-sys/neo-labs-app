...
@section('content')
  ...
  <section aria-labelledby="partner-list-title"><div class="d-flex justify-content-between align-items-center mb-3"><h2 class="h5 mb-0" id="partner-list-title">Hamkorlar ro‘yxati</h2><span class="text-muted">{{ $partners->total() }} ta hamkor</span></div>
    @if($partners->count())<div class="row g-3">@foreach($partners as $partner)<div class="col-12 col-md-6 col-xl-4"><article class="admin-card h-100"><div class="admin-card-body">
      <div class="d-grid place-items-center border rounded-3 p-3 mb-3" style="min-height:110px">@if($partner->url)<a href="{{ $partner->url }}" target="_blank" rel="noopener" aria-label="Hamkor saytini yangi oynada ochish"><img src="{{ media_url($partner->path) }}" alt="Hamkor logotipi" style="max-width:180px;max-height:72px;object-fit:contain"></a>@else<img src="{{ media_url($partner->path) }}" alt="Hamkor logotipi" style="max-width:180px;max-height:72px;object-fit:contain">@endif</div>
